...
def create_text_from_excel(df, gap, line):
    ...
    #         file.write("<Routes>\n")
        ...
        for index, row in df.iterrows():
            ...
            source_file = f"routefiles/{routeNo}.fdb" 
            destination_folder = f"routefolders/{routeNo}" 
            os.makedirs(destination_folder, exist_ok=True)
            destination_file = os.path.join(destination_folder, f"DIS01.chm")
            shutil.copy(source_file, destination_file)

            source_file = f"routefiles/{routeNo}.sdb" 
            destination_folder = f"routefolders/{routeNo}" 
            os.makedirs(destination_folder, exist_ok=True)
            destination_file = os.path.join(destination_folder, f"DIS02.chm")
            shutil.copy(source_file, destination_file)

            source_file = f"routefiles/{routeNo}.rdb" 
            destination_folder = f"routefolders/{routeNo}" 
            os.makedirs(destination_folder, exist_ok=True)
            destination_file = os.path.join(destination_folder, f"DIS03.chm")
            shutil.copy(source_file, destination_file)
